...
OPERADORES  = ["foi respondido com", "foi respondido como"]
...
RETORNAR_TAREFA = "Retornar a Tarefa"
...
OPS_ES = {
    ...
    "foi respondido como": "fue respondido como",
}
...
class CollapsibleGroup(ctk.CTkFrame):
    """Container com título e opção de colapsar/expandir."""

    def __init__(self, parent, title: str, *, collapsible: bool = False, default_open: bool = True):
        super().__init__(parent)
        self.collapsible = bool(collapsible)
        self._expanded = bool(default_open) or not self.collapsible
        ...
        self._title = ctk.CTkLabel(header, text=title, font=font)
        self._title.grid(row=0, column=0, sticky="w")

        self._toggle_var = tk.StringVar(value="−" if self._expanded else "+") if self.collapsible else None
        ...
        if self._expanded:
            self.body.grid(row=1, column=0, sticky="nsew", padx=6, pady=(6, 8))
    ...
    def toggle(self):
        ...
        if self._toggle_var is not None:
            self._toggle_var.set("−" if self._expanded else "+")
...
def group(parent, text, row, col=0, padx=10, pady=(10,6), sticky="we", *, collapsible=False, default_open=True):
    wrapper = CollapsibleGroup(parent, text, collapsible=collapsible, default_open=default_open)
    wrapper.grid(row=row, column=col, sticky=sticky, padx=padx, pady=pady)
    return wrapper.body
...
class LinhaCondicao(ctk.CTkFrame):
    def __init__(self, master, on_change, on_remove):
        ...
        ctk.CTkLabel(self, text="Campo:").grid(row=0, column=0, sticky="w", padx=(4, 6), pady=(4, 2))
        ctk.CTkEntry(self, textvariable=self.var_campo).grid(row=0, column=1, sticky="ew", padx=(0, 4), pady=(4, 2))

        ctk.CTkLabel(self, text="Operador:").grid(row=1, column=0, sticky="w", padx=(4, 6), pady=(0, 2))
        self.cbo_op = ctk.CTkComboBox(
            self,
            values=OPERADORES,
            variable=self.var_op,
            command=lambda *_: self.on_change(),
        )
        self.cbo_op.grid(row=1, column=1, sticky="ew", padx=(0, 4), pady=(0, 2))

        ctk.CTkLabel(self, text="Valor / Resposta:").grid(row=2, column=0, sticky="w", padx=(4, 6), pady=(0, 2))
        ...
        def _trace_op(*_):
            self._sync_operator_choices()
            self.on_change()

        self.var_campo.trace_add("write", lambda *a: self.on_change())
        self.var_valor.trace_add("write", lambda *a: self.on_change())
        self.var_op.trace_add("write", _trace_op)
        self._sync_operator_choices()
    ...
    def to_text(self):
        ...
        if not c:
            return ""
        if o in ("é", "não é", "está em", "foi respondido com", "foi respondido como") and not v:
            return ""
        return _cond_to_text(c, o, v)

    def _sync_operator_choices(self):
        try:
            current = self.var_op.get()
            opts = list(OPERADORES)
            if current and current not in opts:
                opts.append(current)
            self.cbo_op.configure(values=opts)
        except Exception:
            pass
...
class LinhaAcao(ctk.CTkFrame):
    ...
    def __init__(self, master, on_change, on_remove, default_resp="Escritório Externo", default_resp_free="", row_list_key="acao_rows"):
        ...
        self.var_tipo = tk.StringVar(value=ACAO_TIPOS_BASE[0])
        ...
        header = ctk.CTkFrame(self, corner_radius=8, fg_color=("gray86", "#1f1f1f"))
        header.grid(row=0, column=0, columnspan=2, sticky="ew", padx=(4, 4), pady=(4, 2))
        ...
        self.frm_dyn.grid_columnconfigure(0, weight=0)
    ...
    def _type_choices(self):
        opts = list(ACAO_TIPOS_BASE)
        cur = self.var_tipo.get()
    ...
    def _refresh(self):
        ...
        try:
            self.cbo_tipo.configure(values=self._type_choices())
        except Exception:
            pass
        ...
        if t == ACAO_TIPOS_BASE[0]:
            row = 0
            ctk.CTkLabel(self.frm_dyn, text="Tarefa:").grid(row=row, column=0, sticky="w", pady=(0, 2))
            ...
            ctk.CTkLabel(self.frm_dyn, text="Responsável:").grid(row=row, column=0, sticky="w", pady=(2, 2))
            ...
            ctk.CTkLabel(self.frm_dyn, text="Tipo de SLA:").grid(row=row, column=0, sticky="w", pady=(2, 2))
            ...
            ctk.CTkLabel(self.frm_dyn, text="Dias / Feriados:").grid(row=row, column=0, sticky="w", pady=(2, 2))
            sla_line = ctk.CTkFrame(self.frm_dyn, fg_color="transparent")
            sla_line.grid(row=row, column=1, sticky="ew", pady=(2, 2))
            sla_line.grid_columnconfigure(0, weight=0)
            IntSpin(
                sla_line,
                from_=0,
                to=365,
            ...
            self.chk_feriados.grid(row=0, column=1, sticky="w", padx=(12, 0))
            row += 1
            ...
            def _apply_sla_visibility(*_):
                ...
                try:
                    ...
                    pass
                if needs_marco:
                    self.lbl_marco.grid(row=marco_row, column=0, sticky="w", pady=(2, 2))
            ...
            ctk.CTkLabel(self.frm_dyn, text="Status:").grid(row=0, column=0, sticky="w", pady=(0, 2))
            ctk.CTkEntry(self.frm_dyn, textvariable=self.var_status).grid(row=0, column=1, sticky="ew", pady=(0, 2))

        elif t == "Acionar Fluxo":
            ctk.CTkLabel(self.frm_dyn, text="Fluxo:").grid(row=0, column=0, sticky="w", pady=(0, 2))
            ctk.CTkEntry(self.frm_dyn, textvariable=self.var_fluxo).grid(row=0, column=1, sticky="ew", pady=(0, 2))

        elif t == RETORNAR_TAREFA:
            ctk.CTkLabel(self.frm_dyn, text="Retornar a tarefa:").grid(row=0, column=0, sticky="w", pady=(0, 2))
        ...
        # --- MELHORIA UX: Novo tipo de ação ---
            ...
            ctk.CTkLabel(self.frm_dyn, text="Texto:").grid(row=0, column=0, sticky="w", pady=(0, 2))
    ...
    def from_dict(self, d: dict):
        ...
        tipo = d.get("tipo", ACAO_TIPOS_BASE[0])
        texto = d.get("texto", "")
...
class RNBuilder(ctk.CTk):
    def __init__(self):
        ...
        # Coluna 0 (esquerda) agora abriga o construtor; mantém mais largura útil.
        self.content.grid_columnconfigure(0, weight=11, minsize=520)
        # Coluna 1 (direita) exibe pré-visualização + lista final.
        self.content.grid_columnconfigure(1, weight=9, minsize=420)

        self._bind_shortcuts()
        self._last_monitor = None
        self.after(150, self._init_layout_scaling)
    ...
    def _open_mem_manager(self):
        try:
            if getattr(self, "_mem_mgr_win", None) and self._mem_mgr_win.winfo_exists():
                self._mem_mgr_win.focus_force()
                return
        except Exception:
            pass

        win = ctk.CTkToplevel(self)
        self._mem_mgr_win = win
        win.title("Gerenciar Tarefas e Campos")
        ...
        def build_section(col, title, storage, bucket: str):
            frame = ctk.CTkFrame(container, fg_color="transparent")
            frame.grid(row=0, column=col, sticky="nsew", padx=(0, 12) if col == 0 else (12, 0))
            frame.grid_columnconfigure(0, weight=1)
            frame.grid_rowconfigure(1, weight=1)

            ctk.CTkLabel(frame, text=title, font=ctk.CTkFont(weight="bold")).grid(row=0, column=0, sticky="w")

            list_frame = ctk.CTkFrame(frame, fg_color="transparent")
            list_frame.grid(row=1, column=0, sticky="nsew", pady=(6, 6))
            list_frame.grid_columnconfigure(0, weight=1)

            lb = tk.Listbox(list_frame, exportselection=False, height=12)
            ...
            entry.grid(row=2, column=0, sticky="ew", pady=(0, 6))

            btns = ctk.CTkFrame(frame, fg_color="transparent")
            btns.grid(row=3, column=0, sticky="ew")
            btns.grid_columnconfigure((0, 1, 2), weight=1)

            def refresh_list(select_idx: int | None = None):
                lb.delete(0, tk.END)
                for item in storage:
                    lb.insert(tk.END, item)
                if select_idx is not None:
                    size = lb.size()
                    if 0 <= select_idx < size:
                        lb.selection_set(select_idx)
                        lb.see(select_idx)
                    elif size:
                        lb.selection_set(size - 1)
                        lb.see(size - 1)
            ...
            def add_item():
                ...
                if len(value) < 3:
                    messagebox.showwarning("Valor curto", "Informe ao menos 3 caracteres.")
                    return
                if bucket == "task":
                    before = len(storage)
                    self._mem_add_task(value)
                    if len(storage) == before:
                        messagebox.showinfo("Duplicado", "Esta tarefa já está salva.")
                else:
                    before = len(storage)
                    self._mem_add_field(value)
                    if len(storage) == before:
                        messagebox.showinfo("Duplicado", "Este campo já está salvo.")
                entry.delete(0, tk.END)
                refresh_list(len(storage) - 1)
            ...
            def remove_item():
                ...
                if bucket == "task":
                    ...
                    self._refresh_field_combos()
                entry.delete(0, tk.END)
                target = min(idx, len(storage) - 1) if storage else None
                refresh_list(target)
            ...
            def rename_item():
                ...
                if len(value) < 3:
                    messagebox.showwarning("Valor curto", "Informe ao menos 3 caracteres.")
                    return
                if any(value.lower() == it.lower() for i, it in enumerate(storage) if i != idx):
                    messagebox.showinfo("Duplicado", "Já existe um item com este nome.")
                    return
                storage[idx] = value
                if bucket == "task":
                    self._refresh_task_combos()
            ...
            ctk.CTkButton(btns, text="Adicionar", command=add_item).grid(row=0, column=0, padx=(0, 4), sticky="ew")
            ctk.CTkButton(btns, text="Renomear", command=rename_item).grid(row=0, column=1, padx=4, sticky="ew")
            ctk.CTkButton(btns, text="Remover", command=remove_item).grid(row=0, column=2, padx=(4, 0), sticky="ew")
            ...
            def on_select(_event=None):
                ...
                if idx is None:
                    return
                entry.delete(0, tk.END)
                entry.insert(0, storage[idx])

            lb.bind("<<ListboxSelect>>", on_select)
            refresh_list()

        build_section(0, "Tarefas", self._mem_tasks, "task")
        build_section(1, "Campos", self._mem_fields, "field")
    ...
    def _init_layout_scaling(self):
        ...
        if sw and sh:
            try:
                width = max(1120, min(int(sw * 0.85), sw - 160))
                height = max(800, min(int(sh * 0.85), sh - 160))
                self.geometry(f"{width}x{height}")
            except Exception:
                pass
            self._last_monitor = (sw, sh)
        try:
        ...
        try:
            ...
            pass
        try:
            self.bind("<Configure>", self._ensure_monitor_scaling, add="+")
        except Exception:
            pass

    def _ensure_monitor_scaling(self, _event=None):
        try:
            sw = self.winfo_screenwidth()
            sh = self.winfo_screenheight()
        except Exception:
            return
        last = getattr(self, "_last_monitor", None)
        if last == (sw, sh):
            return
        self._last_monitor = (sw, sh)
        try:
    ...
    def _reset_all(self):
        ...
        try:
            ...
            if hasattr(self, "_clear_preview"):
                self._clear_preview()
            if hasattr(self, "_clear_builder"):
                self._clear_builder()
            ...
            if hasattr(self, "_update_preview"):
                self._update_preview()
            for grp_name in ("grp_gatilho", "grp_condicoes", "grp_acoes"):
...
def _attach_builder_to_RNBuilder():
    def _build_rule(self: 'RNBuilder'):
        ...
        self.builder_container.grid_rowconfigure(0, weight=6)
        ...
        self.grp_gatilho = frm_gatilho_group.master
        ...
        self.grp_condicoes = frm_cond_group.master
        ...
        self.grp_acoes = frm_acao_group.master
        ...
        freq_close.grid(row=5, column=0, padx=6, pady=(2, 4), sticky="ew")
        freq_close.grid_columnconfigure(1, weight=1)
        freq_close.grid_columnconfigure(2, weight=1)
        ctk.CTkLabel(freq_close, text="Encerramento:").grid(row=0, column=0, sticky="w")
        ctk.CTkButton(
            freq_close,
            text="Parcial",
            command=lambda: self._insert_frequent_close(parcial=True),
        ).grid(row=0, column=1, sticky="ew", padx=(12, 6))
        ctk.CTkButton(
            freq_close,
            text="Total",
        ...
        freq_cond.grid(row=6, column=0, padx=6, pady=(2, 6), sticky="ew")
        freq_cond.grid_columnconfigure(0, weight=3)
        freq_cond.grid_columnconfigure(1, weight=2)
        ...
        ctk.CTkLabel(freq_cond, text="Resposta:").grid(row=1, column=1, sticky="w", pady=(4, 0), padx=(12, 0))
        ...
        cb_field.grid(row=2, column=0, sticky="ew", pady=(2, 0))
        ...
        ent_resp.grid(row=2, column=1, sticky="ew", pady=(2, 0), padx=(12, 0))
    ...
    def _reset_builder_defaults(self: 'RNBuilder'):
        ...
        if hasattr(self, 'var_freq'):
            try:
                self.var_freq.set("Cadastro")
            except Exception:
                pass
        if hasattr(self, 'var_freq_ret'):
            self.var_freq_ret.set("")
        if hasattr(self, 'var_freq_ret_restart'):
            self.var_freq_ret_restart.set(True)
        if hasattr(self, 'var_freq_cond_field'):
            self.var_freq_cond_field.set("")
        if hasattr(self, 'var_freq_cond_resp'):
    ...
    def _insert_frequent_return(self: 'RNBuilder'):
        try:
            ...
            row = LinhaAcao(self.frm_acoes_body, on_change=self._update_preview,
                            ...
                            default_resp=preset_value, default_resp_free=self.var_resp_preset_free.get(),
                            row_list_key="acao_rows") # Passando a chave da lista
            row.var_tipo.set(RETORNAR_TAREFA); row._refresh()
    ...
    def _insert_frequent_close(self: 'RNBuilder', *, parcial: bool):
        try:
            preset_value = (
                self.var_resp_preset_free.get().strip()
                if self.var_resp_preset.get() == "Texto livre…"
                else self.var_resp_preset.get()
            )
            ...
            row.var_tipo.set("Encerrar Fluxo (Parcial)" if parcial else "Encerrar Fluxo (Total)")
            row._refresh()
            self.acao_rows.append(row)
            self._relayout_acao_rows()
            self._update_preview()
